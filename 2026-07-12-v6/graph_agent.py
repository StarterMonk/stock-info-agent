...
def _agent(state: "State"):
    ...
    # 截断过大的 ToolMessage 内容，节省 Token（原始内容保留在 state 中供图表提取）
    truncated_messages = _truncate_tool_messages(state["messages"])
    resp = llm_with_tools.invoke([sys_msg] + truncated_messages)
...
def _memory_node(state: "State"):
    ...
    if facts:
        ...
        logger.info("[memory_node] 长期记忆已更新 session=%s, facts=%d chars", state['session_id'], len(facts))
    return {"long_memory_facts": facts}
...
def _build_graph():
    ...
    if _GRAPH is None:
        ...
        sqlite_conn = sqlite3.connect(mem.DB_PATH, check_same_thread=False)
...
# ---------------------------------------------------------------------------
# 5. 工具消息截断（节省 LLM Token）
# ---------------------------------------------------------------------------
_MAX_TOOL_MSG_CHARS = 2000


def _truncate_tool_messages(messages):
    """Return a copy of messages with oversized ToolMessage content truncated for LLM consumption."""
    out = []
    for m in messages:
        if isinstance(m, ToolMessage) and len(m.content) > _MAX_TOOL_MSG_CHARS:
            truncated = m.content[:_MAX_TOOL_MSG_CHARS] + f"\n... [truncated, total {len(m.content)} chars]"
            out.append(ToolMessage(content=truncated, tool_call_id=m.tool_call_id))
        else:
            out.append(m)
    return out
...
def run_agent(message: str, session_id: str, long_memory: str = ""):
    ...
    graph = _build_graph()
    cfg = {"configurable": {"thread_id": session_id}, "recursion_limit": 3}
    try:
        result = graph.invoke(
    ...
    # 长期记忆已由 memory 节点持久化；从图状态中读取事实，避免重复抽取
    long_memory_facts = result.get("long_memory_facts", "")
...
def _fallback_run(message, long_memory=""):
    ...
    tool_calls = []
    _tool_results = []  # 存储工具实际返回结果，避免后续重复调用
    chart = None
    lines = []
    ...
    if "profile" in intents:
        ...
        _tool_results.append(("get_profile", {"code": code}, r))
        if "error" not in r:
    ...
    if "history" in intents:
        ...
        _tool_results.append(("get_history", {"code": code, "start_date": s, "end_date": e}, r))
        if "error" not in r:
    ...
    if "intraday" in intents:
        ...
        _tool_results.append(("get_intraday", {"code": code, "date": d, "time": intent.get("time")}, r))
        if "error" not in r:
            chart = {"type": "get_intraday", "code": code, "data": r.get("data", [])}
            lines.append(f"**盘中行情（{d}）**")
    # 使用已存储的工具结果生成 fact 摘要，避免重复调用工具
    facts = "\n".join(_summarize_fact(n, a, r) for n, a, r in _tool_results)
