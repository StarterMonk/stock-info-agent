...
def result_summary(result: dict) -> str:
    """提取工具结果的一行摘要，供 Agent A / graph_agent 共用。"""
...
def get_profile(code: str) -> dict:
    ...
    return _cached(_get_profile_inner, 86400, code)


def _get_profile_inner(code: str) -> dict:
    try:
...
def get_financials(code: str, report_type: str = "资产负债表") -> dict:
    ...
    return _cached(_get_financials_inner, 604800, code, report_type)


def _get_financials_inner(code: str, report_type: str = "资产负债表") -> dict:
    try:
        sym = _sina_symbol(code)
...
def get_dividend(code: str) -> dict:
    ...
    return _cached(_get_dividend_inner, 604800, code)


def _get_dividend_inner(code: str) -> dict:
    try:
