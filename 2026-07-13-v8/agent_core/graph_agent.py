...
def run_agent(message: str, session_id: str, long_memory: str = ""):
    ...
    _month_ago = (_today - datetime.timedelta(days=30)).strftime("%Y%m%d")
    _today_compact = _today.strftime("%Y%m%d")
    system_prompt += (
        f"\n\n【重要】今天是 {_today_str}。"
        f"调用工具时请使用正确的当前日期计算时间范围"
        f"（如最近一个月应为 {_month_ago}~{_today_compact}）。"
    )
    ...
    # RAG 前置检索：本地股票名快照 → 提示注入，帮助 LLM 把用户口头名称映射到 6 位代码
    try:
        rag_hits = stock_search.lookup(message, top_k=3)
        if rag_hits:
            hints = "；".join(f"{h['name']}({h['code']})" for h in rag_hits if h["score"] >= 0.06)
            if hints:
                system_prompt += ("\n\n【本地股票名检索】用户消息疑似指向："
                                  f"{hints}。若后续工具参数需要 code，务必直接使用这些 6 位代码。")
    except Exception as exc:
        logger.warning("RAG 检索注入失败（忽略）：%s", exc)
