import sqlite3, os, uuid, datetime, logging
...
def _conn():
    ...
    if _conn_instance is None:
        _conn_instance = sqlite3.connect(DB, check_same_thread=False)
...
def get_session(sid):
    """获取单个会话信息，不存在时返回 None。"""
    c = _conn(); cur = c.cursor()
    cur.execute("SELECT id,title,created_at,updated_at FROM sessions WHERE id=?", (sid,))
    row = cur.fetchone()
    if row:
        return dict(zip(["id", "title", "created_at", "updated_at"], row))
    return None
