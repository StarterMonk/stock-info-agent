"""
v8 FastAPI 入口：OpenRouter LLM 驱动的 A 股股票信息助手（含价格预测引擎 + 算法分析接口）。

- Agent 执行层：graph_agent（OpenRouter 工具调用闭环，无 LangGraph）
- 短期记忆：session_store（SQLite）；长期记忆：memory_store
- v8 新增：/api/analysis/options、/api/analysis/run（算法池：MA / MA5+线性回归 / EMA / BOLL / SARIMA）
- 前端：static/（Vue 3 单页，分类回答 + 算法选择 + 图表工作台）
- 端口 8008
"""
import os
