...
DB_PATH = os.environ.get("V4_DB_PATH", os.path.join(os.path.dirname(__file__), "v6_sessions.db"))
...
def update_long_memory(session_id: str, new_facts: str):
    ...
    try:
        ...
        content = llm_client.generate(
            ...
            system_prompt=_MEMORY_EXTRACT_PROMPT, temperature=0.2)
        merged = "".join(part.get("text", "") for part in content.get("parts", []))
        merged = merged.strip() or new_facts.strip()
        set_long_memory(session_id, merged)
        return merged
    except Exception as e:
        logger.warning("[memory] 抽取失败，直接追加：%s", e, exc_info=True)
        merged = current + ("\n" if current and not current.endswith("\n") else "") + new_facts.strip()
        set_long_memory(session_id, merged)
        return merged
